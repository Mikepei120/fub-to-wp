...
if (version_compare(PHP_VERSION, '7.4', '<')) {
    add_action('admin_notices', function() {
        echo '<div class="error"><p>FUB Integration requires PHP 7.4 or higher.</p></div>';
    });
    return;
}
...
class FUB_Backend_API {
    ...
    public function __construct() {
        ...
        $this->timeout = 30;
        // Get Supabase anon key securely from remote endpoint
        $this->anon_key = $this->get_secure_credential('supabase_anon_key');
    }
    ...
    public function create_stripe_session_with_account($account_id) {
        $data = array(
            ...
            'success_url' => admin_url('admin.php?page=fub-to-wp&step=basic-settings&payment=success&session_id={CHECKOUT_SESSION_ID}'),
            'cancel_url' => admin_url('admin.php?page=fub-to-wp&step=subscription&canceled=true')
        );
    ...
    private function get_fallback_credential($credential_type) {
        ...
        error_log('FUB Integration Error: Unable to fetch credentials from Supabase Edge Function. Please ensure the Edge Function is deployed.');
        
        // Show admin notice
        add_action('admin_notices', function() use ($credential_type) {
            echo '<div class="error"><p>FUB Integration: Unable to fetch ' . esc_html($credential_type) . ' from secure endpoint. Please contact support.</p></div>';
        });
...
class FUB_Setup_Wizard {
    ...
    private function display_step1_connect_fub() {
        ...
        $is_oauth_connected = $this->oauth_manager->is_connected();
        ...
        <div class="fub-setup-step">
            ...
            <?php if ($is_oauth_connected): ?>
                <div class="notice notice-success inline">
                    ...
                    <?php endif; ?>
                    <p><span class="dashicons dashicons-shield-alt" style="color: green;"></span> <strong>OAuth Connection</strong> (Secure)</p>
                </div>
        ...
        if ($account_id) {
            ...
            <?php else: ?>
                ...
                <div class="fub-oauth-info" style="background: #f8f9fa; border: 2px solid #0073aa; border-radius: 8px; padding: 20px; margin: 20px 0;">
                    <h3 style="margin-top: 0; color: #0073aa;">🔐 Secure OAuth Connection</h3>
                    <ul style="margin: 0;">
                        <li><strong>✅ More Secure:</strong> No API keys stored locally</li>
                        <li><strong>✅ Better Control:</strong> Revoke access anytime from FUB</li>
                        <li><strong>✅ Auto-refresh:</strong> Tokens refresh automatically</li>
                        <li><strong>✅ Granular Permissions:</strong> Only request needed permissions</li>
                    </ul>
                </div>
                            ...
                            $.post(ajaxurl, {
                                ...
                                if (response.success && response.data.auth_url) {
                                    $('#oauth-status').html(
                                        ...
                                        '<p><strong>🚀 Redirecting to Follow Up Boss...</strong></p>' +
                                        '<p>You will be redirected to Follow Up Boss to authorize the connection.</p>' +
                                        '</div>'
                                    );
    ...
    public function handle_validate_api_key() {
        ...
        if ($code === 200 && isset($body['account'])) {
            ...
            if ($subscription_check['success'] && isset($subscription_check['data']['has_subscription']) && $subscription_check['data']['has_subscription']) {
    ...
    private function display_dashboard() {
        ...
        <div class="wrap fub-to-wp-wrap" style="background-color: #F4E6CB !important; padding: 30px; border-radius: 10px; margin: 20px;">
            <h1>FUB Integration Dashboard</h1>
            
            <?php if (!$license_valid): ?>
                <div class="notice notice-error">
                    <p>
                        <strong>FUB Integration:</strong> Your license is inactive or expired. 
        ...
        <script>
        jQuery(document).ready(function($) {
            $('#reset-setup').on('click', function(e) {
                e.preventDefault();
                
                if (confirm('Are you sure you want to change your FUB account? This will reset all settings and require setting up the plugin again with a new FUB API Key.')) {
                    $.post(ajaxurl, {
                        action: 'fub_reset_setup',
                        nonce: '<?php echo wp_create_nonce('fub_admin_nonce'); ?>'
                    }, function(response) {
                        if (response.success) {
                            window.location.href = '<?php echo admin_url('admin.php?page=fub-to-wp'); ?>';
                        } else {
    ...
    public function handle_oauth_proxy_callback() {
        ...
        <div class="wrap">
            ...
            <script>
            jQuery(document).ready(function($) {
                // Validate OAuth connection similar to API key validation
                $.post(ajaxurl, {
                    ...
                    if (response.success) {
                        if (response.data.has_active_subscription) {
                            // Account has active subscription - continue with setup wizard
                            $('#validation-status').html('<p>✅ Active license found!</p><p>Continuing with setup...</p>');
                            // Redirect to basic settings step to continue the wizard
                            setTimeout(function() {
                                window.location.href = '<?php echo admin_url('admin.php?page=fub-to-wp&step=basic-settings&license_active=1'); ?>';
                            }, 1500);
                        } else {
                            // No active subscription - redirect to subscription step
                            $('#validation-status').html('<p>📋 Account validated. Redirecting to subscription setup...</p>');
                            setTimeout(function() {
                                window.location.href = '<?php echo admin_url('admin.php?page=fub-to-wp&step=subscription'); ?>';
                            }, 1500);
                        }
                    } else {
                        $('#validation-status').html('<p>❌ Error: ' + (response.data || 'Unknown error') + '</p><p><a href="<?php echo admin_url('admin.php?page=fub-to-wp'); ?>" class="button">Try Again</a></p>');
                    }
                }).fail(function() {
                    $('#validation-status').html('<p>❌ Connection failed. Please try again.</p>');
                });
            });
            </script>
        </div>
...
class FUB_Integration_SaaS {
    ...
    public function admin_notices() {
        // Admin notices removed - license messages now only shown in dashboard
    }
    ...
    public function settings_page() {
        ...
        ?>
        <div class="wrap fub-to-wp-wrap light-bg">
            <div class="fub-setup-wizard">
                <h1>FUB Integration Settings</h1>
                ...
                <div class="notice notice-warning">
                    <p>
                        ...
                        Analytics show historical data, but new leads will not be sent to Follow Up Boss until you renew your subscription.
                    </p>
                </div>
                <?php endif; ?>
    ...
    public function analytics_page() {
        ...
        <div class="wrap fub-to-wp-wrap light-bg">
            <div class="fub-setup-wizard">
                <h1>FUB Integration Analytics</h1>
    ...
    private function create_fub_lead($lead_data) {
        ...
        if ($account_id) {
            ...
            if ($subscription_check['success']) {
                ...
                $subscription_status = isset($subscription_check['data']['status']) ? $subscription_check['data']['status'] : 'unknown';
...
function fub_to_wp_init_update_checker() {
    ...
    // Private repository authentication - REQUIRED for private repos
    $github_token = fub_get_github_token();
    if ($github_token) {
...
function fub_get_github_token() {
    // Option 1: Try to get from environment variable (most secure)
    $token = getenv('FUB_GITHUB_TOKEN');
    
    // Option 2: Try to get from WordPress constants (defined in wp-config.php)
    if (!$token && defined('FUB_GITHUB_TOKEN')) {
        $token = FUB_GITHUB_TOKEN;
    }
    
    // Option 3: Encrypted token stored in database (fallback)
    if (!$token) {
        $encrypted_token = get_option('fub_github_token_encrypted');
        if ($encrypted_token) {
            $token = fub_decrypt_token($encrypted_token);
        }
    }
